...
**The internals boundary.** The eventized marker is a property keyed by `NAMESPACE` (`Symbol.for('eventize')`) holding `{protocol, keeper, store}`, and that hidden slot _is_ the definition of an emitter. In the types the slot is opaque: the real shape lives in `src/internals.ts`, which `src/index.ts` does not re-export, and everything internal reads it through `internalsOf()` — which holds **the one cast** this boundary costs. Needing a second cast means the boundary is drawn in the wrong place and belongs redrawn, not patched. `npm run check:dts` enforces the consequence: `Eventize` is the only class the published declarations may carry. Reasoning: `src/internals.ts`, `src/asEventized.ts`, `src/constants.ts`.
