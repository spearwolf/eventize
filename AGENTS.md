...
- `off(ε, '*', listenerObject)` is a *targeted* removal, not a bulk one, and `EventStore.removeByEventNameAndListenerObject()` has to special-case the wildcard to make it one: `'*'` is not a key in `namedListeners`, so the branch searched a bucket wildcard listeners can never be in and removed nothing (fixed in v6.1.0). Two boundaries hold it in place, both pinned in `off.spec.ts`: a **named** subscription of the same object survives — `off(ε, listenerObject)` is the form that takes both — and retained state is untouched, which needs no code of its own because `retain()` rejects `'*'`, so the `keeper.remove('*')` that `off()` performs finds nothing. Reference counting behaves as in the named case: `removeSimilarListenersFromArray()` detaches outright, so one call releases a `refCount`-2 registration.
