...
**`EventStore` clone-on-mutate.** `forEach()` walks the live buckets; the protection sits on the mutating side. Four rules hold it up, and each of them has been paid for once:
...
- **Compare buckets by identity and length.** The held count rides on a module-private symbol, which `Object.keys()` and array spread do not see but `Reflect.ownKeys()` and Jest's `toEqual` / `toStrictEqual` do, so a bucket never equals a plain array literal.
