...
- **Compare buckets by identity and length.** The held count rides on a package-private symbol, which `Object.keys()` and array spread do not see but `Reflect.ownKeys()` and Jest's `toEqual` / `toStrictEqual` do, so a bucket never equals a plain array literal. The dedup index rides on a second one.
