...
- **Types:** Generic event-map support — opt-in, fully backwards-compatible. `eventize<TEvents>()`, `eventize.inject<TEvents>()`, and `class extends Eventize<TEvents>` now accept a user-defined event map describing each event's argument tuple. The standalone API (`on` / `once` / `onceAsync` / `emit` / `emitAsync` / `retain` / `retainClear` / `unretain` / `off`) gained typed overloads that narrow against `keyof TEvents` and `TEvents[K]` when the first argument is a typed emitter, and falls back to the v4 loose signatures otherwise. Wrong event names, wrong argument tuples, and unknown keys in a typed listener-object now fail to compile; correct calls get full autocomplete and listener-arg inference. Symbol event names remain accepted on typed emitters as a duck-typing escape hatch (with permissive args), so libraries that mix typed and ad-hoc events keep working. The duck-typing first-class story is unchanged: `eventize()` without a generic, `Eventize` without a generic, plain `{}` passed to `on`/`emit`, and listener-objects with arbitrary method names all behave exactly like v4.
  - New types in `types.ts`: `EventMap` (constraint, intentionally as loose as `object` so user maps don't need an index signature), `DefaultEventMap` (`Record<EventName, any[]>`), `EventKeysOf<T>`, `ArgsFor<T, K>`, `ListenerFor<T, K>`, `EventListenerMethods<T>`, `NonTypedEmitter<T>`. The existing `EventizedObject`, `EventizeApi`, `SubscribeFunc`, and `Eventize` class all gained an `<TEvents extends EventMap = DefaultEventMap>` parameter that defaults to permissive — the v4 type signatures are recovered exactly.
  - The phantom field `[__TEventsBrand]: (events: TEvents) => TEvents` on `EventizedObject` is unexported and never present at runtime; it exists only to make TS preserve `TEvents` invariantly through structural matching, so wrong-name calls actually error instead of silently falling through to the loose overload.
  - `NonTypedEmitter<T>` is the conditional that routes typed emitters _away_ from the loose fallback overloads (resolves to `never` for typed emitters, leaving only the typed overloads that can match).
  - Migration: opt in by passing your event map. Define it as a plain interface — `interface MyEvents { data: [string, number]; ready: [] }`. Do **not** add `extends EventMap`: that would inherit an index signature and widen `keyof MyEvents` back to `string | symbol`, defeating the narrowing. Then `const ε = eventize<MyEvents>()` (or `class extends Eventize<MyEvents>`) and the rest of the API picks up the types automatically. Untyped existing code keeps working unchanged.
  - Spec: `src/typed-events.spec.ts` — 23 specs covering typed `emit` / `on` / `once` / `onceAsync` / `emitAsync` / `retain` / `unretain` / `retainClear` (functional, inject, and class form), the typed listener-object form, the symbol-event escape hatch, full duck-typing fallback when no generic is supplied, and `@ts-expect-error` compile-time assertions for wrong event names, wrong argument tuples, wrong listener arity, and unknown listener-object keys.
- **API:** `Priority` now exposes intuitive aliases — `Critical` (= `AAA`, `1e9`), `High` (= `BB`, `1e6`), and `Normal` (= `Default`, `0`) — so listener priorities can be expressed in plain English instead of the legacy double-letter codes. The previous names (`AAA`, `BB`, `C`, `Default`) are kept as aliases for backwards compatibility, so existing call sites (`on(ε, 'foo', Priority.AAA, fn)`, etc.) continue to work unchanged. The `EventizePriority` interface in `types.ts` was extended with the four new fields. README's _Priorities_ subsection now uses the new names in its example and lists the legacy aliases at the end. `Priority.spec.ts` gained six new specs covering the new constants, the alias equality (`Priority.AAA === Priority.Critical`, `BB === High`, `Default === Normal`), and the relationship chain (`Max > Critical > High > Normal > Low > Min`).
- **Refactor:** Renamed `EventKeeper.emit(...)` to `EventKeeper.replayTo(eventName, listener, sortedEvents?)`, and the `KeeperEvent.emit` closure to `KeeperEvent.replay`. The previous name was misleading — the method does not emit anything new, it collects/replays the *retained* value(s) for a freshly subscribed listener so the listener immediately sees the last emitted state. `EventKeeper` is internal (not exported from `src/index.ts`), so this is not a breaking change for public API consumers; the only call sites — `subscribeTo.ts` and `EventKeeper.spec.ts` — were updated, including the spec's `existingEvents = [{order: 0, emit: …}]` test fixture which now uses `{order: 0, replay: …}` to match the new property name. Behavior is unchanged; the existing 314-test suite still passes.
- **API:** Added public `unretain(emitter, eventName | eventName[])`. Until now there was no supported way to undo `retain()` once it had been called: `retainClear()` only discards the currently stored value but leaves the retain policy active, so subsequent emissions would be re-retained again. `unretain()` is the missing inverse — it drops the stored value **and** removes the event from the keeper's `eventNames` set, so future emits are not retained. Available in all three API shapes (standalone `unretain(obj, …)`, injected `obj.unretain(…)`, and `Eventize#unretain(…)`); follows the strict-mode convention (throws `"object is not eventized"` on a plain object, like `emit`/`off`/`retainClear`). Added `src/unretain.spec.ts` with 12 specs covering policy removal, future-emission behavior, idempotency, array/symbol forms, re-retain after unretain, and the strict-mode error path; the helper `expect2ImplEventizeApi` was extended to assert presence of `.unretain()`.
- **Performance:** `EventStore.add` now inserts new listeners at their sorted position via binary search instead of `arr.push(listener); arr.sort(...)` after every subscription. Insertion drops from `O(n log n)` to `O(log n)` lookup + `O(n)` shift, which only matters with many listeners on the same event/wildcard slot — for typical sub-100-listener usage the change is unobservable. Sort order is unchanged: descending `priority`, then ascending `id` (stable across equal priorities, since `EventListener.id` is a monotonic counter). Added two `EventStore` specs covering mixed-priority insertion order and equal-priority FIFO stability; the existing 314-test suite still passes unmodified.
- **Refactor:** Moved the test helper `expect2ImplEventizeApi` from `src/expect2ImplEventizeApi.ts` to `src/__test-utils__/expect2ImplEventizeApi.ts`. The helper was sitting alongside production sources but is only used by `eventize.spec.ts` and `composition-over-inheritance.spec.ts`; the new location signals its test-only nature. No runtime change — the file is not exported from `src/index.ts` and `tsup.config.js` only bundles `src/index.ts`, so `lib/` is unaffected. The two import paths in the consuming specs were updated; the existing 314-test suite passes unmodified.
- **Docs:** README — added an "Auto-eventize vs. strict mode (intentional asymmetry)" section to the _Creating Emitters_ chapter. Documents that `on`/`once`/`onceAsync`/`retain` auto-eventize a plain object on first use (they _install_ behavior — requiring an explicit `eventize()` call would be pure ceremony), while `emit`/`emitAsync`/`off`/`retainClear` throw `"object is not eventized"` (they _operate on_ an existing emitter — calling them on a plain `{}` is almost always a typo, and silent no-ops would mask the bug). Includes a quick-reference table, the rationale for each side, contrasting code examples, and a cross-reference to `isEventized()` for defensive checks. No code change.
