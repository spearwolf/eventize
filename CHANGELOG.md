...
Everything below is stated against `v5.1.0` — the version consumers actually have. `v6.0.0` is the only `6.x` there is, so this is the whole jump. Twelve breaking changes; the two with the widest reach are every bulk `off()` form now clearing retained state along with listeners, and `on()` and `once()` aggregating by listener identity. [`docs/migration.md`](./docs/migration.md) walks the upgrade with before/after code.
