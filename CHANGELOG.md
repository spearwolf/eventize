...
Two breaking changes, both closing memory-lifecycle gaps the 2026-07-25 audit found: every bulk `off()` form — `off(ε)`, `off(ε, '*')`, and any array containing `'*'` — now clears retained state along with listeners, where it used to leave the keeper untouched; and `once()` no longer deduplicates, so two `once()` calls on the same listener object mean two independent one-shot firings instead of one listener that used to collapse into a permanent subscription. See [`docs/lifecycle.md#migrating-from-v5`](./docs/lifecycle.md#migrating-from-v5) for before/after snippets covering both, and reach for `getRetainedCount(ε)` / `getSubscriptionCount(ε)` to confirm a migration actually worked. Three more breaking fixes, one feature, and the rest of phases 2–3's audit backlog round out the release. `v5.2.0` and `v6.0.0` carry the same date because both were staged the same day from one audit: `v5.2.0` shipped its non-breaking half (the lifecycle fixes and inspection API), `v6.0.0` the breaking half. Anyone skipping straight from `v5.1.0` to `v6.0.0` loses nothing by not stopping at `v5.2.0` — every fix there is already folded into this release.
...
- **Docs:** New "Migrating from v5" section in `docs/lifecycle.md` with before/after snippets for both breaking changes above, and the bundle-size line (`~26.5 kB unminified`, ESM, measured) added near the top of `README.md`.
