...
Everything below is stated against `v5.1.0` — the version consumers actually have. `v6.0.0` is the only `6.x` there is, so this is the whole jump. Twelve breaking changes; the two with the widest reach are every bulk `off()` form now clearing retained state along with listeners, and `once()` no longer deduplicating. [`docs/migration.md`](./docs/migration.md) walks the upgrade with before/after code.

### Breaking

- **`once()` no longer deduplicates.** Two `once()` calls on the same listener object are two independent one-shot subscriptions: two firings, two replays of a retained value, two handles that release separately. They used to collapse into one listener that then fired on every emit and could not be released through its own handles. _Migration:_ drop the duplicate `once()`, or guard the handler against a second call. `on()`'s reference-counted dedup is unchanged.
- **Every bulk `off()` form clears retained state as well as listeners** — `off(ε)`, `off(ε, undefined)`, `off(ε, '*')`, and any array containing `'*'`, `null` or `undefined`. They used to empty the listener registry and leave every retained value and policy in place, so the call that reads as "reset the emitter" was the one that pinned the payloads and still replayed them to the next subscriber. _Migration:_ re-`retain()` and re-`emit()` afterwards, or narrow to `off(ε, eventName)` / `off(ε, [names])`, which are unchanged. Filter runtime-assembled name lists — one nullish element turns the call into a full wipe.
...
- **An `EventListener` constructed directly with a `null` / `undefined` listener** dispatches to nothing instead of throwing, and two of them no longer count as similar. Unreachable through the public API, and the class is no longer exported in either namespace.

Four more are type-only and surface as compile errors rather than as behaviour changes:
...
- **`EventizedObject`'s marker slot is opaque**, so `EventStore`, `EventKeeper` and `EventListener` no longer appear in `lib/index.d.ts` — `grep -c '^declare class'` drops from 4 to 1. The slot's key is a non-exported `unique symbol`, so reading it from outside answered `TS7053` either way; only code that annotated the slot structurally is affected.
...
- **A consumed unsubscribe handle holds neither the emitter nor the listener.** The closure used to keep both for the handle's lifetime, so a kept handle pinned the store, the keeper and every retained payload — under _any_ event name, not just the one it subscribed to. The teardown pattern this project's own docs recommend, `subs.push(on(…))` followed by `subs.forEach((u) => u())`, is exactly the shape that hit it. An _unconsumed_ handle still pins the emitter, by design.
...
- **A retained event is no longer replayed twice when `on()` deduplicates the subscription.** The replay now runs only when the store actually inserted a listener; a deduplicated `on()` is a pure reference-count bump.
...
- `npm run cbt` is a gate that binds. It gained a `typecheck` step (`tsc --noEmit`) — esbuild does not type-check, tsup's dts pass does not either, and `attw --pack` only checks already-emitted declarations — and it now runs `test:coverage`, so `jest.config.ts`'s `coverageThreshold` applies locally and not only in CI. Bare `npm test` stays coverage-free for narrow loops.
...
- TypeScript stays at 5.9: `ts-jest` peers on `typescript: ">=4.3 <7"` and `typescript-eslint` on `<6.1.0`, and neither is worth forcing past with `--legacy-peer-deps` or an `overrides` entry. Tracked in [`docs/backlog.md`](./docs/backlog.md).
...
- New documents: [`docs/lifecycle.md`](./docs/lifecycle.md) (what an emitter holds and what releases it), [`docs/migration.md`](./docs/migration.md) (the v5 → v6 upgrade, worked) and [`docs/backlog.md`](./docs/backlog.md) (open and deliberately deferred items). `remediation-plan.md` and `audit.html` — point-in-time working artifacts — are gone, along with the temporary issue identifiers they handed out.
