...
Everything below is stated against `v5.1.0` — the version consumers actually have. `v6.0.0` is the only `6.x` there is, so this is the whole jump. Nineteen breaking changes — twelve runtime, seven type-only; the two with the widest reach are every bulk `off()` form now clearing retained state along with listeners, and `on()` and `once()` aggregating by listener identity. [`docs/migration.md`](./docs/migration.md) walks the upgrade with before/after code.
...
- **`on()` / `once()` / `onceAsync()` throw on a sparse array of event names.** A hole — `new Array(2)`, or a literal built with an elision — used to be silently skipped by the per-name registration, so `on(ε, ['a', , 'b'], h)` registered only `'a'` and `'b'` instead of throwing, and an array of nothing but holes registered nothing at all: `once(ε, new Array(2), h)` returned a handle for zero subscriptions, and `onceAsync(ε, new Array(2))` returned a promise that never settles — the exact failure the empty-array guard exists to prevent, reached through a length that isn't `0`. All three now throw `subscribeTo() called with insufficient arguments` (`Error.cause: 'sparse-names'`), atomically, before anything is registered. An element explicitly set to `undefined` is a value, not a hole, and is rejected by the entry check in the next entry instead (`Error.cause: 'invalid-name'`). _Migration:_ build the name list densely — `push()`/`filter()` never leave a hole — or check for one with an indexed loop (`for (let i = 0; i < names.length; i++) if (!(i in names))`) before subscribing, the same way a nullish element needs checking before a bulk `off()`.
- **`on()` / `once()` / `onceAsync()` throw on an event name that is not a string or a symbol.** `on(ε, [123], fn)`, `on(ε, [null], fn)`, `on(ε, [[]], fn)` and `on(ε, ['a', undefined, 'b'], fn)` used to file a bucket under that value and count it in `getSubscriptionCount(ε)`; no `emit()` could ever reach it, and for a number `off(ε, 123)` could not remove it either — `isEventName(123)` is false, so the removal falls through to identity matching and finds nothing. The single-name forms had the same hole wherever a priority follows the name: `on(ε, {}, 10, fn)`, `on(ε, null, 10, fn)` and the four-argument `on(ε, 5, 10, fn, ctx)`. All of them now throw `subscribeTo() called with insufficient arguments` (`Error.cause: 'invalid-name'`), atomically, before anything is registered, and a `[name, priority]` tuple's first slot is checked the same way. The catch-all spellings are unaffected — they fill the name slot with `'*'` themselves — and so is `on(ε, 123, fn)`, where `123` is decoded as a priority and never as a name. _Migration:_ filter runtime-assembled names to strings and symbols before subscribing.
- **A method-name subscription without a listener object throws instead of registering.** `on(ε, 'foo', 'handler', null)` — or with `undefined`, or with the fourth argument left off — used to throw `TypeError: Cannot read properties of null` at dispatch time up to `v5.1.0`. It now throws `subscribeTo() called with insufficient arguments` (`Error.cause: 'missing-listener-object'`) at the `on()` call, atomically. The slot cannot be filled later — nothing writes it after registration but `detach()` — so such a subscription is dead for its whole life while counting in `getSubscriptionCount(ε)`, and a `once()` in this shape never reaches the settle hook, which leaves its handle holding the emitter. Late binding is unaffected: it is about the *method*, and an object that does not carry it yet still subscribes and starts dispatching the moment it does. _Migration:_ pass the object, or guard the lookup — `if (obj) on(ε, 'foo', 'handler', obj)`.
...
Seven more are type-only and surface as compile errors rather than as behaviour changes:

- **The listener slot no longer accepts an array, `null` or `undefined`.** `on(ε, ['a','b'])`, `on(ε, null)` and `on(ε, undefined)` compiled and then threw `subscribeTo() called with insufficient arguments`; they are compile errors now. The *trailing* slot is unchanged and still nullish wherever it means a `this` context — `on(ε, 'foo', fn, null)`. Behind a method name it is not; see the next entry. _Migration:_ guard the lookup, or keep the handle `on()` returned.
- **The listener-object slot of the method-name forms is `object`.** `on(ε, 'foo', 'handler', null)` and `on(ε, 'foo', 'handler', undefined)` compiled up to `v5.1.0` and are compile errors now, on all three API surfaces and in the `NamedMethodArgs`, `NamedPriorityMethodArgs` and `CatchAllPriorityMethodArgs` arms of `SubscribeArgs`. It is the type half of the runtime rejection above: the method is resolved late, the object it lives on is not. _Migration:_ guard the lookup before the call.
...
- **`subscribeTo() called with insufficient arguments` now names its cause on `Error.cause`** — `'missing-listener'`, `'not-dispatchable'`, `'empty-method-name'`, `'missing-listener-object'`, `'empty-names'`, `'sparse-names'` or `'invalid-name'` — instead of only on the console via `warn()`. The thrown message is unchanged, so code that matches on the string sees no difference.
- **The unusable-priority rejection carries `Error.cause: 'invalid-priority'`.** It was the one throw in `subscribeTo()` without a cause, so a `catch` block written against the vocabulary had to fall back to matching text for exactly that case — and it is also the one whose message differs (`subscribeTo() called with a NaN priority`), so the collective message match did not cover it either. The message is unchanged.
