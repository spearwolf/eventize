...
An argument error — an empty or sparse array of event names, a `NaN` priority, an `options.signal` that isn't shaped like an `AbortSignal` (no `addEventListener`) — throws synchronously out of `onceAsync()` at the call site, not as a rejection: the signal-shape check and `once()`'s own validation both run outside the promise machinery, so a fire-and-forget call with no `await`/`catch` fails where the mistake is instead of becoming an unhandled rejection, and no subscription is left behind for a malformed `signal`. The one exception is an already-aborted signal: that check runs *before* `once()` is ever called, so `onceAsync(ε, [], {signal: alreadyAborted})` rejects with the abort reason rather than throwing — the argument is never validated in that combination.
...
## emit() / emitAsync() / emitSafe() / emitSafeAsync() / emitStrict() / emitStrictAsync()
...
emitStrict(ε, 'name', a, b)          // …and the failures still reach the caller
const values = await emitStrictAsync(ε, 'load')
```
...
`emitSafe()` and `emitSafeAsync()` (v6.1.0) are the same dispatch with each listener isolated: a synchronous throw is caught, reported through `console.warn` with the event name and the error, and the walk continues. The guarantee is execution, not completeness — no listener can prevent the others from running. It buys you no error object and no error channel, and one throw still leaves the call: `'*'` as an event name or inside a name array. As a bare name it is rejected before any listener runs; inside an array the names ahead of it dispatch first and the call throws when the walk reaches it. Either way the throw comes from the call rather than from a listener, which is why the guard is not between you and it.

The guard is not free, and the cost is not local: guarded and unguarded dispatch are two callbacks reaching one call site inside the walk, shared by every emitter in the process. A process that never calls `emitSafe()` pays nothing; once anything in it calls `emitSafe()` even once, **every** `emit()` in that process pays roughly +29% on a 64-listener dispatch (about 2 ns per listener) — including emitters that code never touches, and including a host application's own emitters when the single `emitSafe()` call sits in a dependency. Reach for it where the isolation is worth it, not by default.

`emitStrict()` and `emitStrictAsync()` (v6.2.0) are the same guarded dispatch with the errors kept instead of spent. Nothing thrown means a normal return; one failure is rethrown unchanged, same error and same stack, so swapping `emit()` for `emitStrict()` leaves an existing `toThrow(…)` assertion intact; two or more arrive as an `AggregateError` holding them in dispatch order, a shape `emit()` could never produce because it aborted at the first. A caller's own error is not a listener failure but is not dropped either: `'*'` still ends the dispatch, and the rejection joins the same list as its last entry — so a lone wildcard still throws the plain `Error` it always did, while a wildcard behind failing listeners no longer erases what they threw. The corrupted-bucket throw is treated the same way.

`emitStrictAsync()` aggregates with `Promise.allSettled` on both levels, the outer collection and the inner unwrap of a listener that returned an array. `Promise.all` reports whichever promise rejected first *in time* and silently drops the rest; `allSettled` returns positionally, so failures come back in dispatch order — synchronous throws first, then rejections. If anything failed the promise rejects and there is no result array at all; otherwise it resolves exactly what `emitAsync()` would have. And it never throws synchronously: a wildcard name, a foreign protocol marker and a corrupted bucket all arrive through the returned promise, which is the one place `emitStrictAsync(…).catch(report)` can see them. `emitAsync()` and `emitSafeAsync()` keep throwing synchronously — deliberate, and changing them would be breaking.

`emitStrict()` shares the guarded callback with `emitSafe()`, so it does not add a second surcharge on top of the one above: a process paying for one pays the same for both.

`emitSafeAsync()` keeps `Promise.all`, so a listener returning a rejected promise still rejects the awaited result. That is not a gap: dispatch is synchronous, so by the time a promise rejects every listener has already run, and a rejection prevents no execution. What it does fix is the paragraph above — a synchronous throw no longer costs the values collected before it. Two behaviours differ from `emit()` and are intended, under all four guarded variants: the retained value is written, because the event was delivered, and a `once()` queued behind a throwing listener is spent, because it now runs. The throwing listener keeps its own subscription in every case.
...
- A listener that throws **on a replay** is the one throw an unguarded dispatch swallows. (`emitAsync()` catches too, to claim the promises it had already collected, but rethrows unchanged. Since v6.1.0 `emitSafe()` / `emitSafeAsync()` catch as well — a second catch site, but an opt-in one: the replay guard applies without anyone asking, the guarded dispatch is something the caller chose by calling a different function.) `publishReplays()` isolates each queued replay: the throw goes to `warn()` (`console.warn`, prefixed `[eventize]`) with the event name and the error, the remaining replays of that batch run in the order they were sorted into, and `on()`/`once()` return their handle for the complete registration. The asymmetry with `emit()` is the point — an `emit()`'s throw belongs to the caller that caused the event, while the caller of `on()` did not produce the retained value and its listeners are already in the store when a replay runs. Up to v5.1.0 the throw escaped out of `on()`, the batch stopped there, and the registered subscriptions had no handle. Consequence for `once()`: the obligation settles only after the listener returns, so a throwing replay leaves it open and the next replay of the same batch fires the listener a second time; a batch in which every replay throws leaves the `once()` armed.
- Since v6.0.0 the same isolation covers an **`async` listener that rejects on a replay**. Such a listener returns a promise before it fails, so `publishReplays()`'s `try`/`catch` sees a successful call; the replay hands `EventListener.apply()` a return-value collector instead, which duck-types the result on `then` and reports a rejection through the same `warn()` — `a retained replay rejected; the batch continues.` with the replayed event name and the rejection reason. Up to v5.1.0 nothing watched that promise and the failure surfaced as an unhandled rejection, which ends the process under Node's default `--unhandled-rejections=throw`, at a call site that had only subscribed. Two differences to the synchronous case: the warning arrives a microtask after the batch finished, and a `once()` is spent rather than left armed — the dispatch saw a listener that returned normally. The regular `emit()` path is deliberately not covered: it makes no isolation promise, and `emitAsync()` is what hands the promises back.
...
- A throwing listener leaves the previously retained value untouched under `emit()` / `emitAsync()` — `keeper.retain()` (`_emitOne` in `emit-api.ts`) runs after `store.forEach()` returns, i.e. after every listener has run, and a throw unwinds before it is reached. Under `emitSafe()` / `emitSafeAsync()` nothing unwinds, so the write happens: same code, opposite outcome, both intended (see the `emitSafe()` paragraphs above). The same ordering means **any** nested `emit()` call — forwarding to a different event or re-emitting the same one — writes its retain state before the call that contains it: nested calls write on unwind, innermost first, outermost last. Forwarding a chain of distinct events (`retain(ε,'a')`, `retain(ε,'b')`, a listener on `'a'` that calls `emit(ε,'b', …)`) retains `'b'` before `'a'`, reversing the emission order. Self-recursion — re-emitting the *same* name — is the special case where it's most surprising, because both nested calls compete for one slot: `retain(ε,'ping')` plus a listener that counts `0 → 1 → 2` via recursive re-emission leaves `0`, not `2`, retained.
